...
export class OrderRepository extends BaseRepository<IOrderModel> implements IOrderRepository {
    ...
    async findAllOrders(page=1, limit=4, userId?: string): Promise<FindOrdersForUser> {
        console.log(userId);
        const query = userId ? {userId} : {};
        const result = await this.model.find(query).skip((page - 1) * limit).limit(limit).sort({ orderDate: -1 });
        const resultCount = await this.model.countDocuments(query);
        return {data:result, total:resultCount};
    }
